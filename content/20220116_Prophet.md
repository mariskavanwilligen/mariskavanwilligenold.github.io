...
grab_cols_reg = ['_reg_' + reg for reg in grab_cols] # output names for used regreggors, so they are not the same as input column name
...
# makes partitioning and runs item forecasts
